...
## Attempted: run migrations in the test fixture (abandoned 2026-08-17)

conftest builds the test schema with `Base.metadata.create_all`, so the
migrations themselves are never executed by any test. That gap caused a real
bug: `search_vector` was created from the model as a plain nullable tsvector,
stayed NULL forever, and every full-text search silently matched nothing while
dev worked fine.

**That bug is fixed** -- the generated expression now lives in the model via
`Computed(...)`, so create_all and the migration produce the same column.

Tried replacing create_all with a session-scoped `alembic upgrade head`.
Abandoned after several rounds. What we learned:

1. `command.downgrade(cfg, "base")` does not work as a reset. Alembic's
   autogenerated downgrades emit `drop_table` but never `DROP TYPE`, so the
   five enum types (patient_sex, trial_status, user_role, node_type,
   criteria_operator) survive and the next upgrade dies on
   "type patient_sex already exists". **The migration chain is not actually
   reversible** -- worth fixing on its own merits.
2. `env.py` unconditionally overwrote `sqlalchemy.url` at import time, so
   `cfg.set_main_option(...)` from a fixture was ignored -- migrations would
   have run against the dev database. Made the default conditional.
3. Replacing the downgrade with `DROP SCHEMA public CASCADE` did not help.
   Verified: schema empty before the run, empty after, single test, still
   raised DuplicateObjectError in 0.25s. The error is not reflecting real
   catalog state -- possibly a stale bytecode/module issue that was never
   isolated. Unresolved.

Not blocking anything. The `jsonb_path_ops` GIN index and the raw-SQL parts of
both week-2 migrations remain untested by the suite; that is the residual risk.
